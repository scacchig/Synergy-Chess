...
class GoratschinChess:
    ...
    _engines = [None, None, None, None, None, None, None, None]

    # The current move decided by the engine. None when it doesn't know yet
    _moves = [None, None, None, None, None, None, None, None]
        
    # The current infos
    _info = [None, None, None, None, None, None, None, None]

    _pos = "position startpos"

    # The current score of move decided by the engine. None when it doesn't know yet
    _scores = [None, None, None, None, None, None, None, None]
    _scores_white = [None, None, None, None, None, None, None, None] # from white's view
    ...
    def start(self):
        ...
        for i in range(0, len(self._engines)):
            try:
                ...
                if i == 7:
                    emit_and_log("info string started engine 7 as counselor7      (" + engineName + ")")
    ...
    def _mainloop(self):
        ...
        while not exitFlag:
            ...
            if userCommand == "uci":
                ...
                self._moves = [None, None, None, None, None, None, None, None]
                self._scores = [None, None, None, None, None, None, None, None]
    ...
    def _decide(self, index):
        ...
        if None not in self._moves:
            ...
            diff = self._scores[boss] - self._scores[counselor7]
            self._printResult(boss, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[boss]
            self._printResult(counselor7, boss, diff)
            ...
            diff = self._scores[counselor] - self._scores[counselor7]
            self._printResult(counselor, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor]
            self._printResult(counselor7, counselor, diff)
            ...
            diff = self._scores[counselor2] - self._scores[counselor7]
            self._printResult(counselor2, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor2]
            self._printResult(counselor7, counselor2, diff)
            ...
            diff = self._scores[counselor3] - self._scores[counselor7]
            self._printResult(counselor3, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor3]
            self._printResult(counselor7, counselor3, diff)
            ...
            diff = self._scores[counselor4] - self._scores[counselor7]
            self._printResult(counselor4, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor4]
            self._printResult(counselor7, counselor4, diff)
            ...
            diff = self._scores[counselor5] - self._scores[counselor7]
            self._printResult(counselor5, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor5]
            self._printResult(counselor7, counselor5, diff)
            diff = self._scores[counselor6] - self._scores[counselor7]
            self._printResult(counselor6, counselor7, diff)
            diff = self._scores[counselor7] - self._scores[counselor6]
            self._printResult(counselor7, counselor6, diff)
            if self._moves[boss] == self._moves[counselor] == self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor6]:
                ...
                decider = counselor3
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor2] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor2] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor2] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor2] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor3] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor3] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor4] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[counselor] == self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor2] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor2] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor2] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor3] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor3] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor4] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor4] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor3] == self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                ...
                emit_and_log("info string listening to counselor4: C4 OK by {:2.2f}".format(diff))
                decider = counselor4
            elif self._moves[boss] == self._moves[counselor] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor2] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[boss] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to boss: B OK by {:2.2f}".format(diff))
                decider = boss
            elif self._moves[counselor] == self._moves[counselor2] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor: C OK by {:2.2f}".format(diff))
                decider = counselor
            elif self._moves[counselor2] == self._moves[counselor3] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor2] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor2: C2 OK by {:2.2f}".format(diff))
                decider = counselor2
            elif self._moves[counselor3] == self._moves[counselor4] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor3] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor3] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor3: C3 OK by {:2.2f}".format(diff))
                decider = counselor3
            elif self._moves[counselor4] == self._moves[counselor5] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor4: C4 OK by {:2.2f}".format(diff))
                decider = counselor4
            elif self._moves[counselor4] == self._moves[counselor6] == self._moves[counselor7]:
                emit_and_log("info string listening to counselor4: C4 OK by {:2.2f}".format(diff))
                decider = counselor4
            elif self._moves[counselor5] == self._moves[counselor6] == self._moves[counselor7]:
                ...
                emit_and_log("info string listening to counselor7: C7 OK by {:2.2f}".format(diff))
                decider = counselor7
    ...
    # initialize infos
    def init_infos(self):
        self.listenedTo = [0, 0, 0, 0, 0, 0, 0, 0]
        self.agreed = 0
    ...
    def _printResult(self, boss, counselor7, diff):
          emit_and_log("info string final results - boss: bm " +  str(self._moves[boss]) + " sc " + str(self._scores[boss])
                + " - counselor7: bm " + str(self._moves[counselor7]) + " sc " + str(self._scores[counselor7])
                + " diff: {:2.2f}".format(diff))
    ...
    def _printStats(self):
        ...
        winCounselor7, drawCounselor7, lossCounselor7 = get_win_draw_loss_percentages(self._scores_white[7])
        emit_and_log("info string Counselor7 best move: " + str(self._moves[7]) + " score: " + str(self._scores[7])
                      + " white {:2.1f}% win, {:2.1f}% draw, {:2.1f}% loss".format(winCounselor7, drawCounselor7, lossCounselor7))


        emit_and_log("info string listen stats [Boss, Counselor, Counselor2, Counselor3, Counselor4, Counselor5, Counselor6, Counselor7] " + str(self.listenedTo))
